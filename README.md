...
### Особенности шрифтов  
Каждый глиф описывается структурой, содержащей ширину, межсимвольный отступ и битовые строки - это пропорциональный шрифт. Для поддержки надстрочных элементов (например, точек над `Ё` и `Й`) в начале каждого глифа добавлены две строки. Это позволило сохранить оригинальное расположение символов, не смещая их вниз.
